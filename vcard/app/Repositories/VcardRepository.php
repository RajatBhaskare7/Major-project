    ...
    public function update($input, $vcard)
    {
        ...
        try {
            ...
            if (isset($input['part']) && $input['part'] == 'appointments') {
                if ($input['is_paid'] == 1) {
                    if (! getUserSettingValue('stripe_enable', getLogInUserId()) && ! getUserSettingValue('phonepe_enable', getLogInUserId()) && ! getUserSettingValue('paytack_enable', getLogInUserId()) && ! getUserSettingValue(
                        'paypal_enable',
                        getLogInUserId()
                    )) {
